...
export const MessageZone: React.FC<MessageZoneProps> = ({ location, className = '' }) => {
  ...
  const renderMessage = (text: string) => {
    ...
    let processedText = text.replace(/\[icon:([^:]+)(?::\s*([^\]]+))?\]/g, (match, iconName, textColor) => {
      ...
      if (iconDef) {
        return `<span class="inline-flex items-center gap-2 ${colorClass} min-w-0">${iconDef.svg}</span>`;
      }
      
      // Fallback for unknown icons
      return `<span class="inline-flex items-center gap-2 ${colorClass} min-w-0">📱 <span>${iconName}</span></span>`;
    });
    ...
    processedText = processedText.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, linkText, url) => {
      return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="text-blue-500 hover:text-blue-400 underline inline-flex items-center gap-2 min-w-0">${linkText} ${iconDefinitions['ExternalLink'].svg}</a>`;
    });
  ...
  return (
    <div className={`w-full rounded-lg p-4 ${className}`}>
      <div 
        className="text-gray-700 dark:text-white text-sm flex items-center gap-3 min-w-0"
